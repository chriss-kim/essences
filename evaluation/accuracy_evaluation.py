...
for test_data in test_data_list:
    ...
    #     test_result['correct_agent_list'] = test_data['target_agents']              ## 정답 Agent list
    #     test_result['selected_agent_list'] = json.loads(res.json())['agent_list']   ## 응답 Agent list
    #     test_result['selected_agent_list'] = test_data['selected_agents']
    #
    # else:
    #     test_result = {}
...
for cal_accuracy_target in test_result_list:
    correct_agent_list = cal_accuracy_target['correct_agent_list']    ## 정답 Agent list
    selected_agent_list = cal_accuracy_target['selected_agent_list']  ## 응답 Agent list

    correct_negative = [item for item in agent_pool if item not in correct_agent_list]   ## 정답 - 선택되지 않았어야 할 Agnet
    predict_negative = [item for item in agent_pool if item not in selected_agent_list]  ## 예측 - 선택되지 않은 Agent
    ...
    recall.append(len(fp_list) / (len(fp_list)+len(tn_list)) )     # (= FP / FP + TN)
...
## Draw ROC-curve, precision-recall Graph
...
plt.subplot(1, 2, 2)
plt.plot(recall, precision, marker='o', color='g', label='Precision-Recall Curve')
plt.xlabel("Recall")
...
plt.subplot(1, 2, 2)
plt.plot(recall, precision, color='orange', label=f'PR curve (AUC = {pr_auc:.3f})')
plt.xlabel("Recall")
plt.ylabel("Precision")
plt.title("Precision-Recall Curve")
plt.legend()
